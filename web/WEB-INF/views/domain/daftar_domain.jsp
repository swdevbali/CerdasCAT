...
<c:if test="${user_credential!=null}" >
	...
	<div id="pageContent">
	  ...
	  <div class="wikistyle">
	    <h1 id="Startinguptheproject"><!-- InstanceBeginEditable name="judul_modul" -->Daftar Domain <!-- InstanceEndEditable --></h1>
	    <!-- InstanceBeginEditable name="isi_modul" -->
	    <h1></h1>
	    <a href="<%=Config.base_url%>index/Domain/input/-1">Tambah Data</a>
		<table width="100%" id="rounded-corner" summary="2007 Major IT Companies' Profit">
          <thead>
            <tr>
              <th scope="col" class="rounded-company">No.</th>
              <th scope="col" class="rounded-q1">Domain</th>
          ...
          <tbody>
            <c:forEach items="${row}" var="item" varStatus="status" >
              <tr>
                <td>${status.count}</td>
                <td>${item.domain}</td>
                <td><a href="<%=Config.base_url%>index/Domain/input/${item.iddomain}">Ubah</a> <a href="<%=Config.base_url%>index/Domain/delete/${item.iddomain}"
