...
<c:if test="${user_credential!=null}" >
	...
	<div id="pageContent">
	  ...
	  <div class="wikistyle">
	    <h1 id="Startinguptheproject"><!-- InstanceBeginEditable name="judul_modul" -->Penentuan Domain Soal Bagi Peserta Test <!-- InstanceEndEditable --></h1>
	    <!-- InstanceBeginEditable name="isi_modul" --> <a href="<%=Config.base_url%>index/Domain/input/-1">Tambah Data</a>
        <table width="100%" id="rounded-corner">
          <thead>
            <tr>
              <th scope="col" class="rounded-company">No.</th>
              <th scope="col" class="rounded-q1">Peserta Test </th>
              <th scope="col" class="rounded-q4">Aksi</th>
            </tr>
          </thead>
          <tfoot>
            <tr>
              <td colspan="2" class="rounded-foot-left"><%=Pagination.createLinks(pagenum)%></td>
          ...
          <tbody>
            <c:forEach items="${row}" var="item" varStatus="status" >
              <tr>
                <td>${status.count}</td>
                <td>${item.username}</td>
                <td><a href="<%=Config.base_url%>index/PenentuanDomainSoal/input/${item.id}">Penentuan</a> </td>
              </tr>
            </c:forEach>
          </tbody>
        </table>
