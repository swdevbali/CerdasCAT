...
<c:if test="${user_credential!=null}" >
	...
	<div id="pageContent">
	  ...
	  <div class="wikistyle">
	    <h1 id="Startinguptheproject"><!-- InstanceBeginEditable name="judul_modul" -->Form Penentuan Domain bagi Peserta Test <!-- InstanceEndEditable --></h1>
	    <!-- InstanceBeginEditable name="isi_modul" -->

	    <form name="form1" method="post" action="<%=Config.base_url%>index/PenentuanDomainSoal/save">
	    <p id="aIntroductiona">
	      <label>
	      <input type="submit" name="Submit" value="Simpan">
	      </label>
	    </p>
	      <p>Pilih Domain yang Akan dipergunakan </p>
          <table width="100%" id="rounded-corner" summary="2007 Major IT Companies' Profit">
            ...
            <c:forEach items="${row_soal}" var="item" varStatus="status" >
                <tr>
                  <td><label>
                    <input name="chk_soal_${item.iddomain}" type="checkbox" id="chk_soal_${item.iddomain}" value="selected" 
					<%
					   String checked="";
                                           DomainModel domainModel = ((DomainModel) pageContext.getAttribute("item"));
                                           String sql = "SELECT iddomain FROM peserta_test_domain where idpeserta_test = '" + session.getAttribute("idpeserta_test") + "' and iddomain='"+ domainModel.getIddomain() +"'";
